...
suite('Debug Adapter', () => {
	...
	const DEBUG_ADAPTER = './out/atmelDebugAdapter.js';


	let dc: DebugClient;

	setup( () => {
		dc = new DebugClient('atbackend', DEBUG_ADAPTER, 'atbackend');
		return dc.start();
	});
	...
	suite('initialize', () => {
		...
		// 		assert.equal(response.body.supportsConfigurationDoneRequest, false);
		// 	});
		// });
